...
RUN apt update
RUN apt install -y git dvb-apps \
...
RUN git clone https://github.com/tvheadend/tvheadend.git /tvh-build && \
    ...
    AUTOBUILD_CONFIGURE_EXTRA="--enable-ccache --enable-ffmpeg_static --enable-hdhomerun_static" ./Autobuild.sh -t bionic-amd64 -j$(nproc) && \
    make && \
    make install && \
    rm -rf /tvh-build && \
